...
- `ext/skills/browser-control/SKILL.md`: the top-level onscreen skill for inspecting, navigating, and interacting with open browser windows through the top-level numeric-id `space.browser` helpers, including typed reference-marked page captures and ref-targeted browser actions without per-window handles or explicit sync calls, plus guidance to inspect one link or image target through `detail(id, ref)` instead of printing every link URL in `content(...)`, to treat the lean default content shape as the normal low-token mode, and to rely on browser-like bare-host normalization for `open(...)` and typed `navigate(...)`
